...
async function createLocation(page: Page, name: string) {
  ...
  const createForm = page.getByTestId("location-create");
  await expect(createForm).toBeVisible();

  await createForm.getByTestId("location-name").fill(name);
  await expect(createForm.getByTestId("location-active")).toBeVisible();
  await createForm.getByTestId("location-save").click();
...
async function pickTutorOption(
  ...
  const options = await tutorSelect.evaluate((element) => {
  ...
  const valid = options.filter(
    (option) =>
      ...
      option.label.toLowerCase() !== "select a tutor" &&
      option.label !== disallowedLabel
  );

  if (preferredLabel) {
    const preferred = valid.find((option) => option.label === preferredLabel);
    if (preferred) {
      await tutorSelect.selectOption(preferred.value);
  ...
  await tutorSelect.selectOption(valid[0].value);
