...
import LocationFilter from "@/app/manager/schedule/location-filter";
import OperatingHoursForm from "@/app/manager/schedule/operating-hours-form";
import RecurringSessionForm from "@/app/manager/schedule/recurring-session-form";
import WeekCalendar from "@/app/manager/schedule/week-calendar";
...
export default async function ManagerSchedulePage({
  ...
  return (
    <div className="space-y-6" data-testid="admin-schedule-entry">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <Link
            href={`/admin/schedule?week=${prevWeek}${selectedLocationId ? `&location=${selectedLocationId}` : ""}`}
            className={cn(buttonVariants({ variant: "outline", size: "sm" }))}
          >
            Previous week
          </Link>
          <Link
            href={`/admin/schedule?week=${nextWeek}${selectedLocationId ? `&location=${selectedLocationId}` : ""}`}
            className={cn(buttonVariants({ variant: "outline", size: "sm" }))}
          >
            Next week
          </Link>
